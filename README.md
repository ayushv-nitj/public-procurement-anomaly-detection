...
- [How the Pipeline Works (Step by Step)](#how-the-pipeline-works-step-by-step)
  - [Phase 1: Data Input](#phase-1-synthetic-data-generation)
...
Your application now supports **5 different data sources**, each producing **different results**:

## 1. CSV (Default)
**File:** `data/contracts.csv`  
**Command:** `python run_pipeline.py --source csv --filepath data/contracts.csv`

- **Size:** 500 contracts
- **Data:** Generated by `python data/generate_data.py`
- **Randomness:** ✅ **Generates NEW data every time you run the generator**
- **Seed:** Uses current timestamp (shown in output)
- **Typical Results:** 65-70 anomalies, 13-15 high risk
- **Use Case:** Main dataset for demos

## 2. JSON Format
**File:** `data/sample_contracts.json`  
**Command:** `python run_pipeline.py --source json --filepath data/sample_contracts.json`

- **Size:** 50 contracts
- **Vendors:** Tata, Infosys, Wipro, HCL, Tech Mahindra, L&T, Bharat Electronics, etc.
- **Departments:** Defence, IT, Health, Education, Railways, Power
- **Anomalies:** ~8 detected, ~5 high risk
- **Special:** Contains suspicious vendors like "Bharatt Electronics Ltd" (typo)
- **Use Case:** Demonstrate JSON format support

## 3. XML Format
**File:** `data/sample_contracts.xml`  
**Command:** `python run_pipeline.py --source xml --filepath data/sample_contracts.xml`

- **Size:** 50 contracts
- **Vendors:** Accenture, Capgemini, Oracle, IBM, SAP, Microsoft, AWS, Google Cloud, etc.
- **Departments:** Finance, Commerce, IT, Education
- **Anomalies:** ~10 detected, ~4 high risk
- **Special:** Contains suspicious vendor "Accenturre India" (typo) with rapid-fire contracts
- **Use Case:** Demonstrate XML format support + **different data than JSON**

## 4. Synthetic Generation
**Command:** `python run_pipeline.py --source synthetic`

- **Size:** 500 contracts
- **Data:** Generates fresh data on-the-fly
- **Randomness:** **Different data every time**
- **Seed:** Uses current timestamp
- **Output:** Shows seed used: `Using random seed: 1776825141 (data will be different each run)`
- **Typical Results:** 65-70 anomalies, 13-15 high risk (varies each run)
- **Use Case:** Show that data is generated dynamically

## 5. API (Government Data)
**Command:** 
```bash
...
- **Size:** Configurable (use --limit parameter)
- **Data:** Live government procurement data
- **Requirements:** API key from data.gov.in (requires approval)
- **Use Case:** Production deployment with real data
